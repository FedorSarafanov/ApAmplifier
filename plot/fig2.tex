...
\begin{document}
	\begin{tikzpicture}
	  ...
	  %          {$\pgfmathprintnumber[frac, frac denom=32,frac whole=false]{\pgfmathresult}\pi$}
	     ...
	     % \xdef\C{5e-8}
	     % \xdef\R{13000}
		\addplot +[mark=x, mark options={scale=1.5, line width=1pt}, line width=0.2pt,blue] plot [error bars/.cd, y dir = both, y explicit] table 
		[			    	    	    	
			x expr=\thisrow{f},
			% y expr=((\thisrow{uout}/233*2)-6)*1.9+6,
			y expr=\thisrow{uout}/233*2,
			% y error=dz
		] 
		{../data/common_emitter_freqamp.dat};

		\addlegendentry{С конденсатором}

		\addplot +[mark=*, mark size=1.2pt, line width=0.2pt,red] plot [error bars/.cd, y dir = both, y explicit] table 
		[			    	    	    	
			% x=f, 
			% y=uoutc, 
			x expr=\thisrow{f},
			y expr=\thisrow{uoutc}/233*2,
			% y error=dz
		] 
		{../data/common_emitter_freqamp.dat};
...
% Coefficients (with 95% confidence bounds):
       ...
       % p2 =      -324.1  (-384.3, -263.9)
	     ...
	     % \addplot [samples=100, domain=8:75] {55.27*x-324.1};

  % p1 =       16.57  (16.28, 16.86)
       % p2 =      -81.71  (-109.8, -53.63)
	     % \addplot [samples=100, domain=8:235] {16.51*x-81.71};
